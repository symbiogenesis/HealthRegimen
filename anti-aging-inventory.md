...
**EFFORT is a tag, not a score:** `trivial` (<$300/yr and <5 min/wk) · `low` · `moderate` · `high` (>$2,000/yr out of pocket, or >4 h/wk). The thresholds are annual out-of-pocket cost, which is why tirzepatide sits at `moderate` on a covered price and would be `high` at self-pay, and why the generic lipid pair at well under $200/year stays trivial while bempedoic acid does not.

**How the prices in this document are worked out.** Assume ordinary US employer commercial insurance, and take the **lowest** of four numbers: the plan copay, the manufacturer copay card, a discount-card or cash-pharmacy price (GoodRx, SingleCare, Cost Plus), and retail. List price is never the number — it is what nobody pays, and quoting it inflates exactly the comparisons this document is built on. Two consequences do most of the work. A manufacturer copay card is worth nothing unless the plan already covers the drug, so it never rescues an item the payer criteria exclude — which is every PCSK9 inhibitor here. And anything taken without the diagnosis it is labelled for is uncovered by construction: an SGLT2 inhibitor at 38 with no diabetes, kidney disease or heart failure, and tadalafil taken for anything other than erectile dysfunction, both price at the discount-card rate with no card and no copay to fall back on. For those rows **generic status is the whole of the affordability question**, which is why it can decide which member of a class you take.
...
**On the effort tag:** the binding constraint for every member of this family is prior authorization and cash price, not administration. All four sit at `high` for that reason — enlicitide included, despite listing at ~$315/month against ~$500–600 for the injectables, because the payer criteria are the same across the class: a documented failure of statin plus ezetimibe at a baseline LDL-C at or above 70 mg/dL. Nothing in the discount ecosystem reaches past that gate. Coupon pricing on enlicitide lands at ~$327/month, which is *above* list and therefore no help at all, and its $15-per-fill savings card is void for anyone the plan does not already cover — so a card that would make it a $180/year drug for a covered patient is worth nothing to an uncovered one. Cheapest door into the family is a ~$3,780/year cash door. But note the ranking that follows if you strip cost out: inclisiran is **two injections a year**, which is a lighter administration burden than a daily pill. The oral advantage over the monoclonals is real; the oral advantage over inclisiran is not.
...
**Sequence:** ApoB + Lp(a) → **rosuvastatin 10 mg and ezetimibe 10 mg together** → reassess ApoB at 8 weeks → escalate within the PCSK9 family only if off target or Lp(a) is high. Both generics together cost about $12–15/month on discount-card pricing, or a pair of tier-1 copays if the plan is generous, and they get most people to target; reaching for a $6,000/year drug before trying a $150/year one is the definition of a poor bang-for-buck decision. Buy them as **two separate tablets** — the fixed-dose rosuvastatin/ezetimibe combination runs ~$51/month, three to four times the two generics bought apart, for a convenience that is one extra tablet a day. Starting both at once rather than titrating the statin first is what RACING supports, and it tested the exact regimen recommended here: rosuvastatin 10 mg plus ezetimibe 10 mg against rosuvastatin 20 mg alone, n=3,780. The 3-year composite came in at 9.1% against 9.9% (absolute difference −0.78%), non-inferior, while the combination put 72% of patients under LDL-C 70 mg/dL at three years against 58% on monotherapy, with intolerance-driven discontinuation or dose reduction at 4.8% against 8.2%. Two mechanisms at moderate doses beat one at a high dose on tolerability and land lower.

**Why <60 is the commitment and <30 is only a row.** The two targets score differently because the second slice is taken from an already-reduced baseline and costs roughly 25× more per year to reach: both generics together run ~$150/year at the best available price, and the cheapest PCSK9 inhibitor — enlicitide, at a $315/month list price — is ~$3,780/year, which no payer will cover for you, since the criteria across the class require documented failure of statin plus ezetimibe at a baseline LDL-C you will not have. A savings card does not close that gap either; every one of them is conditional on the coverage the criteria deny.
...
| **6** | 3×2 | Dapagliflozin / empagliflozin / canagliflozin | SGLT2 | monitor | trivial |
...
**Which agent: dapagliflozin.** Between the other two the choice is a forced trade — canagliflozin holds the lifespan data and carries the CANVAS amputation signal, empagliflozin has the clean human safety record and no lifespan data, and the amputation signal is human evidence while the lifespan signal is mouse. Dapagliflozin sits outside that trade. Two of the three non-diabetic trials this row's E3 rests on, **DAPA-HF and DAPA-CKD, are dapagliflozin trials**; dapagliflozin is the SGLT2 arm in VITAL-H, the study designed to remove the objection that caps the score; and DECLARE-TIMI 58 found no amputation excess, so the one property empagliflozin is picked for is not exclusive to it. The agent with the human non-diabetic evidence base and the pending geroprotection readout is also the one that is generic. Dose 10 mg, which is what DAPA-HF, DAPA-CKD and DECLARE used.

**And the price gap is the size of the decision, not a rounding note.** Generic dapagliflozin arrived in April 2026 and runs roughly **$5–20/month** — about $15 per 90 days at a cash pharmacy, ~$16/month on a discount card, ~$20–25 on GoodRx. Empagliflozin has no generic and will not have one before 2027–28; a coupon takes Jardiance to ~$255–460/month against ~$550 retail. Canagliflozin is brand-only in practice at ~$580–725/month. Because this use is off-label at 38 — no diabetes, no CKD, no heart failure — none of it is covered and no manufacturer card applies, so those are the numbers actually paid. That is a 15–50× spread across three drugs hitting one target, and it is why the row is tagged `trivial`. A generic empagliflozin closes the gap in 2027–28 without reversing the choice: price is the fourth argument here and the first three do not move with a patent. DECLARE-TIMI 58 stays the one trial in the class run mostly in primary prevention, VITAL-H stays a dapagliflozin readout, and empagliflozin's mortality result comes from EMPA-REG's established-CVD diabetics rather than from the non-diabetic trials, where the significant all-cause reductions are dapagliflozin's. What would move the agent choice is a diagnosis, not a patent.

**Cheap is not a reason to take it.** At ~$150/year — roughly what the ezetimibe costs — nothing holding this row at 6 is touched: B2 is the binding constraint, the benefit in a metabolically healthy 38-year-old is speculative and largely redundant with tirzepatide, and the drug still cannot share a calendar with the FMD cycles. A cheap speculative drug is a speculative drug. What the price does settle is that cost is no part of the reason to skip it — the decision rests entirely on the evidence and on what a continuous agent does to the shape of the year.
...
| 6 | SGLT2 inhibitors *(dapagliflozin)* | incretin | monitor *(not with fasting cycles)* | trivial |
