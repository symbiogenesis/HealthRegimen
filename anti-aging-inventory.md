...
The mechanism underneath that finding is the best cell-level argument the whole structure has, and it is worth naming even though it is mechanism rather than result. Myonuclei added during a hypertrophy block are retained long after the volume comes down, which is what makes re-expression after a period of reduced training cheap rather than a rebuild from baseline. Muscle is therefore the tissue where alternation costs least, because the expensive part of the previous block is still there when the next one opens.
