...
**Why:** E5 — three decades of RCTs with hard outcomes. B5 — moves a typical 38-year-old from roughly ApoB 100 to roughly 60 for about $60/year, the largest and cheapest increment available anywhere in this document. Hormone-stack reassurance: the 2024 meta-analysis (21 studies, 9,879 patients) found statins lower *total* testosterone by ~13 ng/dL in RCTs, not below the normal range, with no change in *free* testosterone, LH, estradiol or SHBG.
...
**The mechanism underneath that is why muscle is the tissue block cycling suits best.** Myonuclei added during a hypertrophy block are retained well past the point where volume drops, so re-expression after a reduced-volume period is cheap rather than a rebuild from baseline — the expensive part of the previous block is still present when the next one opens. That is mechanism rather than an outcome and it is graded as such, but it is the clearest cell-level reason the alternation does not ratchet lean mass downward. Bone has no equivalent: its remodeling cycle runs months with formation lagging resorption, so it integrates the year instead of alternating with it, which is why loading stays in both blocks.
