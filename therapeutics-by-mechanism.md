...
`E` = evidence (1–5) · `B` = benefit to a healthy 38-year-old US male (1–5) · safety is a veto flag, not a score · effort sits outside the score.

**Prices** assume ordinary US employer commercial insurance and are the lowest of plan copay, manufacturer copay card, discount-card or cash-pharmacy price, and retail — never list. The [framework](anti-aging-inventory.md#part-1--the-framework) works through why that distinction decides several rows: a copay card is void without coverage, and anything used off its label is uncovered by construction and prices at the discount-card rate.
...
**4 × 4 = 16** · OK · trivial · ~$75 per scan cash, ~$300/yr at the cycle cadence — body composition is not a covered indication, so this is out of pocket whatever the plan
...
**Why:** E3 — hearing loss is the largest single modifiable dementia risk factor in the Lancet Commission model, but the causal chain rests on cohort data. B2 — at 38 the value is a baseline to detect drift against.

### Grip dynamometer
**3 × 2 = 6** · OK · trivial · ~$30 once
**Target:** whole-body strength proxy · **Cadence:** weekly · **Cautions:** none
**Why:** E3 — observational. B2 — it will not change your outcome by itself, but it is the cheapest early-warning signal for the lean-mass problem GLP-1s create, and the only readout in the whole measurement plan that runs weekly. Grip moves before DEXA does, which is what the cadence is for: it catches a lean-mass problem inside an anti-growth block rather than at the boundary scan that closes it.
...
**5 × 1 = 5** · OK · low · ~$100–150 cash, rarely covered as screening · **DEFER TO 45–50**
**Target:** subclinical atherosclerosis · **Cadence:** revisit at 45–50 · **Cautions:** none
**Why:** E5 for risk stratification. B1 at your age — most 38-year-olds score zero, and zero does not exclude soft plaque, so the result will not change what you do. Earlier only if ApoB or Lp(a) come back high.
...
**The practical position:** treat <60 as the commitment and don't back away from a lower number the generics hand you for free. Rosuvastatin 10 mg plus ezetimibe lands many people near 45. Take that; just don't buy the last 15 points. The arithmetic on that last clause: both generics together run ~$150/year at the best price available, and enlicitide lists at $315/month — ~$3,780/year, or about $150,000 undiscounted over the 40 years the cumulative-exposure case is asking you to commit to. There is no coupon that changes this. Enlicitide's discount-card price is *above* list, and its $15-per-fill savings card is conditional on the plan covering the drug, which the payer criteria are specifically written to prevent here.
...
**5 × 5 = 25** · MONITOR · trivial · Rx, ~$2–10/mo
...
**Why:** E5 — three decades of RCTs with hard outcomes. B5 — moves a typical 38-year-old from roughly ApoB 100 to roughly 60 for the price of the statin alone, ~$25–120/year at a cash pharmacy or on a discount card and often a $0–10 tier-1 copay, the largest and cheapest increment available anywhere in this document. Hormone-stack reassurance: the 2024 meta-analysis (21 studies, 9,879 patients) found statins lower *total* testosterone by ~13 ng/dL in RCTs, not below the normal range, with no change in *free* testosterone, LH, estradiol or SHBG.
...
**Take it with the statin rather than after it.** RACING randomised 3,780 patients with established ASCVD to rosuvastatin 10 mg plus ezetimibe 10 mg versus rosuvastatin 20 mg alone — the same pairing recommended here, tested head to head. The 3-year composite of cardiovascular death, major cardiovascular event or stroke was 9.1% against 9.9% (172 versus 186 patients; absolute difference −0.78%, 90% CI −2.39 to 0.83), meeting non-inferiority. The combination arm got 72% of patients under LDL-C 70 mg/dL at three years against 58% on monotherapy, and intolerance-driven discontinuation or dose reduction ran 4.8% against 8.2%. Two mechanisms at moderate doses outperform one at a high dose on tolerability and land lower. RACING was run in established ASCVD, so the outcome equivalence is borrowed rather than demonstrated in primary prevention — but the LDL and tolerability results are what the sequencing decision turns on, and those transfer.
...
#### Bempedoic acid
**4 × 4 = 16** · MONITOR · high *(at cash; the copay card only works once statin intolerance is documented)* · Rx, ~$231/mo on the best coupon · **only if statin-intolerant**
...
**Why:** E4 — CLEAR Outcomes (n=13,970, 40.6 months) showed a 13% MACE reduction. A *post-hoc* analysis of its primary-prevention cohort (n=4,206) found 27% lower all-cause mortality (HR 0.73, 0.54–0.98) alongside a 30% reduction in the primary composite — the strongest mortality result in this family from anything resembling a prevention population, and one that did not appear in the overall trial, which is why it is read as a signal rather than a finding. That population was also statin-intolerant, mean age 68, and two-thirds diabetic. B4 — meaningful apoB reduction *when it is the only thing on board*.
...
**What it actually costs, which is the one place the condition on this row cuts in its favour.** There is no generic, and retail is ~$517/month; the best coupon takes it to ~$231. But the trigger for this row — documented statin intolerance — is also the payer criterion, so in the only scenario where you would take it the drug is on-label and coverable, and the manufacturer copay card then puts it near **$10 per fill** for a commercially insured patient. That is the reverse of the PCSK9 situation, where the gate and the card are set against each other. The cost objection to bempedoic acid is therefore weak, and the `high` effort tag describes the cash case rather than the case in which you would actually reach for it. What decides the row is the 5.4% above, not the price.

**Nexlizet** is bempedoic acid and ezetimibe in one tablet, ~$431/month retail and ~$231 on the best coupon, with no generic. Since generic ezetimibe is ~$10/month, the marginal thing being purchased is the 5.4% component. If statin plus ezetimibe leaves you off target, the evidence points up the ladder to PCSK9 — the same trial that produced the 5.4% figure showed enlicitide beating bempedoic acid, ezetimibe and their combination — not sideways to ACL.
...
**Why:** E5 — FOURIER and ODYSSEY established hard-outcome benefit, and VESALIUS-CV extended it to primary prevention: 12,257 patients with no prior MI or stroke, median 4.6 years, 3-point MACE down 25% on top of maximally tolerated statin (HR 0.75, 95% CI 0.65–0.86, 1.8% absolute), 4-point MACE down 19% (HR 0.81, 0.73–0.89) and MI alone down 36% (HR 0.64, 0.52–0.79). All-cause mortality was 7.9% against 9.7% and is explicitly hypothesis-generating on hierarchical testing, so this is an events benefit demonstrated in people who have not had an event, not a mortality claim. B5 — same target as the statin, deeper. Scores identically to the generic statin because on the merits it is that good; the entire difference is the effort tag and the coverage gate. Prior authorisation generally requires established ASCVD or HeFH, which you do not have — and note that VESALIUS-CV's "primary prevention" still meant documented atherosclerosis or high-risk diabetes plus LDL-C ≥90 mg/dL, which is a long way from a healthy 38-year-old. The drug is proven further than it is available to you, and further than it is needed by you.
...
**The cheapest way into this family, and still not cheap.** At a $315/month list price it undercuts the monoclonals roughly two-fold, and its label is broad — primary hypercholesterolaemia, as an adjunct to diet and maximally tolerated statin, rather than gated on ASCVD or familial hypercholesterolaemia. The label is not the gate, though. Payer criteria require a documented eight-week failure of high-intensity statin plus ezetimibe with a baseline LDL-C at or above 70 mg/dL, which is a threshold a healthy 38-year-old on both generics is nowhere near. Treat it as a ~$3,780/year cash item until that changes.

**Neither discount route reaches it.** A newly launched brand has no discount-card leverage — coupon pricing on enlicitide sits at ~$327/month, above the $315 list, so the card is worse than paying the manufacturer directly. The savings card is the one that would matter, at $15 per fill up to $300 per fill, but it is void for anyone whose plan is not already paying, which is the entire population this row is about. For a *covered* patient the drug is a $180–1,200/year item on a tier-3 copay; for you it is $3,780. The gate and the discount are the same variable, which is why the effort tag does not move with the list price.
...
#### Tirzepatide
**4 × 4 = 16** · MONITOR · moderate *(~$25/mo covered with the manufacturer card; `high` at the $299–449/mo self-pay price)* · Rx · **CONTINUE**
...
**Cautions:** **with an SGLT2 inhibitor, any week of vomiting or minimal intake becomes a euglycemic ketoacidosis risk**; declare it before any procedure under sedation — a hold is risk-stratified rather than automatic, and weekly dosing is one of the risk factors; interacts with low-protein protocols
**Why:** E4 — SELECT showed a 20% MACE reduction with semaglutide (HR 0.80, 0.72–0.90) in 17,604 overweight adults without diabetes, along with all-cause death at HR 0.81 (0.71–0.93). Read the population before reading it across: every participant was 45 or older with *established* cardiovascular disease, so this is secondary prevention in people who happen not to be diabetic, not evidence in the healthy. For tirzepatide specifically the hard-outcome trial has not reported. SURMOUNT-MMO (n≈15,000, obesity without diabetes, all-cause death inside the primary composite, completing October 2027) is the first incretin outcomes trial to cover primary as well as secondary prevention, and is the readout that would move this row to E5. B4 — the best-evidenced pharmacological item in your stack. Pair with protein and lifting or the lean-mass cost is real. This is the item that most constrains everything else on the list.
...
**Brown adipose is not one of the reasons.** GIP receptor agonism increases energy expenditure in rodents with a brown-fat component to the effect, and it is a plausible piece of why a dual agonist outperforms GLP-1 alone. It does not transfer cleanly to a 38-year-old human: adult BAT is a few tens of grams, highly variable, and human energy-expenditure measurements on incretin agonists have not shown thermogenesis beyond what the weight loss itself predicts. The human effect is dominated by intake. Continued dosing is worth defending on regain and on maintained cardiometabolic control — both measured in humans on hard endpoints — rather than on thermogenesis.

**What "covered price" means here, because this is the widest spread in the document.** Roughly 43–45% of US commercial plans cover an incretin for obesity at all; those that do place it on tier 3 or 4 behind prior authorisation at a $100–600 monthly copay, which the manufacturer savings card takes to about $25 for a commercially insured patient. Off formulary the fallback is the manufacturer's own self-pay channel — single-dose vials rather than autoinjectors, $299/month at 2.5 mg, $399 at 5 mg and $449 at 7.5 mg and above, conditional on refilling within 45 days of the last delivery. So the same drug is a ~$300/year item or a ~$5,400/year one depending on one formulary decision, which is the whole distance between the `moderate` and `high` tags. No discount card closes it: there is no generic, and coupon networks do not price brand incretins meaningfully. Appealing a denial with documented BMI and comorbidity is worth more than any purchasing tactic available on this row.
...
**Cautions:** carries the whole GLP-1 caution set — declare it before any procedure under sedation, euglycemic ketoacidosis risk alongside an SGLT2 inhibitor in any week of vomiting or minimal intake — plus two of its own: dysesthesia, and a resting heart rate rise of roughly 5–10 bpm
...
**3 × 2 = 6** · MONITOR · trivial · Rx, generic dapagliflozin ~$5–20/mo · **optional; incompatible with fasting cycles**
...
**Why:** E3 — ITP canagliflozin gave **+14% median lifespan in males only**, the largest male effect in ITP history, concordant across all three test sites, with 90th-percentile survival up 9%. On the human side the decisive fact is that **DAPA-HF, DAPA-CKD and EMPA-KIDNEY enrolled non-diabetics and the benefit held in that subgroup** — so this is not a glucose-lowering effect riding on baseline hyperglycemia. Large cohorts also show ~30% lower dementia incidence versus DPP-4 inhibitors, consistent across meta-analyses but observational. What still caps it at E3: every trial is in T2D, HF or CKD, where much of the benefit is hemodynamic and renal, and none is in metabolically healthy adults. B2 — for a 38-year-old with no HF, CKD or T2D and ApoB already controlled by a statin, the absolute benefit is speculative and substantially redundant with tirzepatide, which delivers the metabolic effect with hard-outcome data behind it. The source table ranked this #1 on a 12/12 gerotherapeutics score; that score means "most deserves a trial," which entails that the human aging evidence does not yet exist.

**The blood pressure effect is real but is not an ARB, and cannot be used to drop one.** Pooled across trials SGLT2 inhibition gives about **−4/−2 mmHg office and −3.5/−1.8 mmHg on 24-hour ABPM**; Tikkanen's dedicated empagliflozin ABPM study landed at −3.4 mmHg 24-hour systolic at 10 mg and −4.2 at 25 mg. Two things shrink that here. Much of the mechanism is glycosuria-linked osmotic diuresis on top of natriuresis, so it attenuates at euglycemia — in the non-diabetic arms of DAPA-HF and EMPEROR-Reduced the systolic separation was nearer 2 mmHg than 4. And telmisartan 40–80 mg delivers 10–13 mmHg systolic in stage 1 hypertension, which is the size that actually closes a 120 → 110 gap; the SGLT2 inhibitor covers a quarter to a third of it. The mechanisms are not redundant — volume and arterial stiffness against RAS blockade — so they stack cleanly, and there is a minor favourable interaction in that an ARB pushes potassium up on an already potassium-rich vegan diet while SGLT2 inhibition nudges it down. That is an argument for combining them if the number ever demands two agents, not for substituting one. Substituting also trades away the only antihypertensive class with a lifespan signal in normotensive animals (§10) and moves the monitoring load in the wrong direction: the ARB is conditional on the home cuff, costs one potassium and creatinine check, and is *held* through an FMD cycle, where an SGLT2 inhibitor is a standing drug carrying a ketone meter and is mutually exclusive with those cycles outright.
...
**And the two sides of that choice are not the same kind of thing.** This drug is a *state* — a continuous caloric-restriction mimetic that holds the anti-growth setting down for as long as it is taken. The FMD cycles are *transitions*, and the regenerative results behind [growth cycling](anti-aging-inventory.md#part-4--growth-cycling) attach to the change rather than to either setting. Taking the drug in place of the cycles therefore does not swap one anti-growth lever for another; it runs the block continuously anti-growth and removes every transition from the year. That is a design cost the ketoacidosis arithmetic is silent about, and it is a second reason this row stays optional for you rather than becoming the default content of an anti-growth block.

**Which agent: dapagliflozin 10 mg.** Between the other two the choice is a forced trade — the lifespan data is canagliflozin, the clean safety record is empagliflozin, which carries no CANVAS-style amputation signal, and the amputation signal is human while the lifespan signal is mouse. Dapagliflozin is not inside that trade. **Two of the three non-diabetic trials this row's E3 rests on are its own** — DAPA-HF and DAPA-CKD — so the human evidence that makes this more than a glucose drug is largely dapagliflozin evidence in the first place. It is the SGLT2 arm in VITAL-H, the trial named below as the one that would settle the score, so choosing it is also choosing the molecule whose readout will actually be about the drug you took. And DECLARE-TIMI 58 (n=17,160) found no amputation excess, which means empagliflozin's one distinguishing property is not distinguishing. Dose 10 mg — the dose used in DAPA-HF, DAPA-CKD and DECLARE.

**The price difference is 15–50×, and it is decisive because nothing here is covered.** Generic dapagliflozin reached the US market in April 2026 and now runs about **$5–20/month**: ~$15 per 90 days at a cash mail pharmacy, ~$16/month on a discount card, ~$20–25 on a GoodRx coupon against an $834 undiscounted retail. Empagliflozin has no generic — the compound patent is spent but formulation and indication cover run out into 2027–28, and a coupon only takes Jardiance to ~$255–460/month against ~$550 retail. Canagliflozin has an approved generic that is not actually being sold, leaving Invokana at ~$580–725/month. None of these three is covered for a 38-year-old with no diabetes, CKD or heart failure, and the manufacturer cards that would zero out a Jardiance copay all require the plan to be paying in the first place — so the coupon column is the whole of the comparison. About $150/year against $3,000–8,000/year, for three drugs on one target with no efficacy data separating them in this population.

**A generic empagliflozin would not reverse the choice.** The gap closes when its exclusivities lapse in 2027–28, and the other three reasons are untouched by that. Trial provenance is fixed. DECLARE-TIMI 58 stays the closest thing this class has to a population resembling yours — 40.6% with established atherosclerosis, against 99.2% in EMPA-REG — so the one trial run mostly in primary prevention is a dapagliflozin trial. VITAL-H will still be reporting on dapagliflozin, and that argument strengthens as the readout approaches rather than fading. The mortality record does not close it for empagliflozin either: its headline all-cause figure is EMPA-REG's HR 0.68, drawn from the T2D-with-established-CVD population furthest from you, while in the trials that enrolled non-diabetics the significant all-cause reductions are dapagliflozin's (DAPA-CKD HR 0.69, 0.53–0.88) and empagliflozin's are not (EMPA-KIDNEY HR 0.87, 0.70–1.08; EMPEROR-Reduced neutral on CV and all-cause death where DAPA-HF reduced CV death). Do not read that as molecular superiority — the EMPEROR investigators call the heterogeneity chance, there is no head-to-head and there will not be one, and the pooled class estimate is HR 0.86. Read it as the weaker claim that does the work here: nothing makes empagliflozin the better molecule once both are cheap, so the tiebreakers stay provenance and the pending trial. **What would move the agent choice is a diagnosis, not a patent** — with T2D, CKD or heart failure the drug is covered, price stops being a variable, and the agent is picked on label and formulary by whoever is treating it.

**Cheap does not promote it.** The row is `trivial` on effort and still scores 6, because price was never what held it down. B2 is: no heart failure, no CKD, no diabetes, and the metabolic effect substantially redundant with tirzepatide, which has hard-outcome data. What the generic settles is a narrower thing — cost can no longer appear anywhere in the reason for skipping it, so the choice stands entirely on the E3 cap and on the calendar argument above.
...
**Why:** E3 — three RCTs now, in journals of a different class from anything else in the supplement section. Mount et al., *Nature Medicine*, 2026 (n=90, Maastricht/Copenhagen/Wageningen): after an 8-week low-energy diet, 24 weeks of pasteurized MucT gave 1.2 ± 0.7 kg regain versus 3.2 ± 0.4 kg on placebo (P=0.012); net weight loss from baseline 3.1 kg greater (P=0.009); no serious treatment-related adverse events. Depommier 2019 (*Nature Medicine*, n=32) and Zhang 2025 (*Cell Metabolism*) point the same direction.

**The sponsorship is not the axis that separates this from Pendulum, and the row should not pretend otherwise.** Mount was funded by The Akkermansia Company, which also appears among the conducting institutions — so this is industry-funded work with academic partners, not independent replication. What actually distinguishes it is that the trials are randomised and placebo-controlled, run at named academic centres across three countries, published in *Nature Medicine* and *Cell Metabolism*, and repeated across three separate studies rather than resting on one proof-of-concept. That is a real difference in evidence quality; it is not a difference in who paid. B3 — weight regain after weight loss is the single unsolved problem for anyone on a GLP-1, and this is the only supplement on the entire inventory with a positive RCT aimed at it.
...
**Why:** E2 — TAME remains unfunded; diabetes RCTs do not transfer to a normoglycemic 38-year-old. B2 — speculative. It competes with cardiorespiratory training, which scores 20 against its 4, for the same adaptation.
...
**Why:** E2 — the best animal lifespan data of any molecule, but PEARL, the only RCT in normative aging, **missed its primary endpoint**, and its one significant efficacy finding was lean mass in **women** at 10 mg (n=114, ages 50–85). B3 — real if it translates. For a 38-year-old man PEARL establishes safety and little else.
...
**It is also a state rather than a transition**, which is the structural argument against it independent of the score. It deepens the anti-growth setting for as long as it is on board and adds no alternation of its own, so what it buys is more of the arm the block already has — against a washout that constrains the boundary and a lipid effect that has to be watched on the row scoring 25. The default block runs without it.
...
**It is the only transition in this section; everything else here is a state.** Rapamycin, dietary composition and the SGLT2 inhibitor in §3b all hold mTORC1 and IGF-1 signalling down for as long as they are administered. A fasting cycle depletes and then repopulates, and the regenerative half is the refeeding. That is why it is not interchangeable with the other anti-growth levers even when the ketoacidosis conflict forces a choice between them, and why it belongs at the *end* of an anti-growth block rather than scattered through it — placed at weeks 19–20 of the [calendar](anti-aging-inventory.md#45-a-concrete-calendar-if-you-run-it), the refeed and the resumption of training volume are one event.

**The gate is the refeed, not the fast.** On a GLP-1 the catabolic half is easy and the anabolic half is the one the drug works against, so the failure mode is a cycle that prunes without regrowing. Protein back to 1.6 g/kg/day within 48 hours of ending a cycle, logged, is the condition for running the next one. Hold any ARB across the fasting days, and never run these alongside an SGLT2 inhibitor.
...
**Why:** E3 — RCTs are consistent but measure surrogates (strength, lean mass, function) rather than mortality. B5 on the second clause of the [benefit scale](anti-aging-inventory.md#the-scoring-model) — it does not move a cause of death by itself, but it is what decides whether the GLP-1's weight loss comes off fat or off muscle, so its failure degrades the tirzepatide row, the bone row and the whole growth-cycling structure at once.
...
**Take that finding functionally and not mechanistically.** The tempting explanation is myonuclear permanence, and in humans it fails: a systematic review and meta-analysis of human and animal studies found exercise-induced myonuclei were not retained over human detraining periods of 12–48 weeks — a range that spans an entire anti-growth block — while remaining elevated in rodents, and pointed to epigenetic regulation instead. Later human work finds retention in type II fibres, so the question is open, but nothing here should lean on it. The maintenance-volume result stands on its own and is what the reduced-volume block is built on.

**Bone is the tissue where the alternation reaches least, and the timescales are why.** A bone remodeling unit runs roughly 120–200 days start to finish, with resorption taking about two weeks and formation about thirteen — so formation lags resorption by months and the whole cycle outlasts a 12-week block. Bone integrates the year rather than alternating with it, which is why impact and axial loading stay in both blocks rather than coming down with the hypertrophy volume, and why the DXA Z-score is the pass condition rather than a reassurance.
...
**The floor is an average, not a daily minimum, and the difference is what lets a fasting cycle sit inside it.** Lean mass integrates over weeks; five FMD days are 6% of a 12-week block, and the per-meal ceiling above governs distribution within a day at a given intake rather than whether a short deficit can be amortised across a block. So the fasting cycles in [§5](#5-mtor-and-nutrient-sensing--the-anti-growth-arm) do not breach this row. What can breach it is the refeed: on a GLP-1 the hard half of a fasting cycle is the eating afterwards, and a cycle whose refeed does not happen has taken the lean-mass debit and left the regenerative credit. Protein back to 1.6 g/kg/day within 48 hours of ending a cycle is the condition for running another.
...
**Target:** phase separation of mTORC1/IGF-1 signalling · **Measure:** DEXA at each phase boundary; grip weekly; IGF-1 and fasting insulin at each phase midpoint; ApoB
**Cautions:** an SGLT2 inhibitor and the FMD cycles cannot occupy the same block; do not compress the washout between blocks
**Why:** E2 — no human trial has tested block alternation of pro- and anti-growth interventions against either arm run continuously. The supporting logic is mechanistic plus indirect: FMD is already a cycle protocol with human trial infrastructure; fasting/refeeding cycles drove HSC self-renewal in mice (Cheng 2014); transient rapamycin produced persistent benefit in mice (Bitto 2016); and exercise periodization is the same idea in a domain where it is uncontroversial. B3 — and the 3 is covering two claims of different strength that should not be collapsed.

**The smaller claim is avoided cost.** The structure recovers the muscle, bone and repair capacity a continuously anti-growth protocol gives up, at no drug expense, and converts three of the phase conflicts in Part 3 from exclusions into calendar entries. On this claim alone the row really is a scheduling structure whose value is already counted in the rows it schedules.

**The larger claim is that the transition is itself the active ingredient**, and it is not reducible to those rows. Cheng's regenerative result is triggered by fasting *and refeeding* — a state neither arm produces while it is running — which means an intervention that alternates can reach an endpoint that neither setting held continuously can. That reading also separates the two kinds of item in §5: a fasting cycle is a *transition*, while rapamycin, an SGLT2 inhibitor and dietary composition are *states* that deepen the block for as long as they are applied. Substituting a state for a transition is not a like-for-like exchange.

E2 is what holds the product at 6, and it holds down the second claim specifically — mechanism plus adjacent animal results, with no human trial. Evidence that alternation beats either arm run continuously raises E and the score follows; evidence that only the avoided cost is real leaves the number where it is and makes this genuinely a scheduling row.

#### rhGH-containing pro-growth block (TRIIM / TRIIM-X style)
**1 × 2 = 2** · MONITOR · high · trial is ages 40–80; you are 38
*(rhGH + DHEA + metformin)*
...
**Why:** E1 — original TRIIM was n=9, single-arm, **uncontrolled**, with ~2.5 years of epigenetic age reversal measured by clocks that are not validated mortality surrogates. Seven years on, no peer-reviewed TRIIM-X results exist; the only public data is a preliminary "sneak peek" from a Foresight Institute talk in May 2026. The trial is participant-funded and sponsor-run, which is a structurally weak evidence generator regardless of the underlying biology. B2 — thymic involution at 38 is modest and the immunosenescence rationale is built on 65-year-olds.
...
**Practical position:** the thymic-regeneration goal is the interesting part, and rhGH is the weakest available route to it. You become eligible for NCT04375657 at 40, which is two years away; if you want the trial, the honest reason to enroll is contributing data, not receiving treatment. Meanwhile FMD cycles target overlapping regenerative biology with better evidence and no drug cost.
...
**Why:** E1 — no human outcome data. B1 — senescent-cell burden at 38 is low, so there is little to clear. Right idea, wrong decade. Revisit at 60.
...
**Why:** E2 — small human trials on surrogates. B1 — no demonstrated outcome benefit in a healthy 38-year-old. Three separate rows in the source for one mechanism.
...
**Why:** E2 — human trials exist and measure surrogates over short horizons; the mood and alertness effects are the best-supported thing about it and are acute rather than durable. B1 — no route to a measurable outcome at 38, and the brown-adipose argument fails here for the same reason it fails for GIP agonism in §3a: adult BAT is a few tens of grams and human energy expenditure does not move beyond what the intervention's other effects predict.
...
**5 × 4 = 20** · MONITOR · trivial · lifestyle; telmisartan ~$11–19/mo on a discount card, or a tier-1 copay, if needed
...
**Why:** E5 — extremely well studied. B1 — and the evidence says it does not help you. ASPREE showed net harm in healthy older adults and there is no primary-prevention indication at 38 without established risk. A high evidence score attached to a finding of no benefit.

#### Tadalafil, low-dose daily
**2 × 2 = 4** · OK · trivial · Rx, generic, ~$12/month at 5 mg on a discount card — no copay applies, since this indication is not one of the three it is labelled for
**Target:** PDE5 → cGMP · **Measure:** home BP series — it is a weak antihypertensive
**Cautions:** **nitrates in any form, absolutely, including whatever is handed to you in an emergency department for chest pain** — carry it on the medication list; additive hypotension with alpha-blockers and during fasting cycles; berberine and grapefruit raise exposure through CYP3A4 and P-gp
**Why:** E2 — a clean mechanism, a drug-target Mendelian randomisation pointing the right way, and an observational literature of more than a million men that is large, consistent and structurally confounded, against a randomised record that is uniformly null. B2 — the mechanism it would plausibly work through is blood pressure and endothelial function, and both are already occupied by rows scoring 20; none of its three approved indications applies to you.

**The observational signal is real, and its size is not.** Three studies, in increasing order of how hard each works to control confounding:

| Design | Comparator | All-cause mortality |
|---|---|---|
| Pooled meta-analysis, 16 cohorts, n = 1,257,759, median 4.3 y | mostly non-users | **RR 0.70** (0.56–0.87) |
| TriNetX claims, 509,788 men with ED and 1,075,908 with LUTS | non-users | **RR 0.66** tadalafil / 0.76 sildenafil |
| Swedish national registers, 16,548 vs 1,994, mean 5.8 y | **alprostadil** | **HR 0.88** (0.79–0.98) |

**The gradient across those rows is the finding.** Alprostadil is the only comparator in the literature that also has erectile dysfunction and is also seeking treatment for it, which holds partnered status, sexual capability, motivation, healthcare engagement and general vigour roughly constant — and against it a 30–34% mortality reduction becomes 12%, barely clearing significance. Even 0.88 is an upper bound, because alprostadil is second-line: it is what men get *after* PDE5 inhibitors fail, which enriches that arm for diabetes, post-prostatectomy status and severe vascular disease. The meta-analysis authors name the problem themselves — healthy-user bias, indication bias, no socioeconomic data, no dose-response, I² = 89% — and the pooled MACE estimate of 0.78 (0.69–0.89) rests on components that do not individually reach significance (MI 0.78, 0.60–1.02; heart failure 0.84, 0.66–1.06).

**Melanoma is the calibration check.** The same literature returns a PDE5i–melanoma association at RR 1.12 (1.03–1.33), driven by sildenafil (1.26) with tadalafil null, and confined to *in situ* disease (1.31) with nothing for localised or metastatic melanoma. Nobody believes an erectile dysfunction drug causes stage-0 melanoma; the association indexes affluence, sun exposure and access to a dermatologist. That machinery is running in the mortality analyses too, pointing the other way, and there is no way to switch it off inside a claims database.

**The exposure in the epidemiology is not the exposure being proposed.** Essentially all of it is on-demand dosing — intermittent 10–20 mg tadalafil, 50–100 mg sildenafil, a few times a week at most. The mortality association has never been demonstrated at 2.5–5 mg daily, so a daily protocol borrows a result from a different dose and a different pattern.

**What the genetics add.** Drug-target Mendelian randomisation using cis-eQTLs for *PDE5A* puts higher enzyme expression at CHD OR 1.22 (1.06–1.40) and MI OR 1.27 (1.09–1.48) in blood, replicating in aortic tissue at 1.17 and 1.15. The direction is right — more enzyme, more cGMP degradation, worse outcomes — and this is the strongest causal evidence available for the target. It remains a single-gene eQTL instrument with acknowledged horizontal pleiotropy, and what it argues for is *lifelong* reduced PDE5 activity, which is the exposure-time discount that governs [every row in the genetics file](genetic-pathways.md#pde5-loss-of-function).

**The randomised record is where it fails.** Sildenafil was developed as an antianginal and failed in trials; the erectile indication was the salvage, so the cardiovascular hypothesis has been tested head-on once already and lost. RELAX randomised 216 HFpEF patients for 24 weeks and returned null on peak VO₂, null on six-minute walk, null on the clinical composite, null on LV mass and diastolic parameters — with worse renal function and larger rises in NT-proBNP, endothelin-1 and uric acid in the sildenafil arm. MAKROTAD gave high-dose tadalafil to well-controlled type 2 diabetics in crossover and was **neutral on insulin resistance**, its primary endpoint, with movement confined to HbA1c, endothelial function and steatosis markers. Over a million people of observational data, and not one randomised hard-outcome event. That is the same evidence shape as [vitamin K](#vitamin-k1-phylloquinone-and-k2-mk-7-mk-4) below: impeccable mechanism, surrogates that move exactly as predicted, endpoints that do not.

**The dementia claim is weaker still, and it is the one most often quoted.** A 2021 network-medicine paper mining 7.23 million insurance records put sildenafil at HR 0.31 for Alzheimer's. NIA's own active-comparator analysis — PDE5 inhibitors against a different drug class in pulmonary arterial hypertension, n > 13,000 — found no difference. Later meta-analyses recover a signal for sildenafil and **not** for tadalafil, which is the opposite of what the blood-brain-barrier argument for tadalafil predicts.

**Safety is the part that holds up, and it is the strongest thing about the row.** Open-label extensions of 5 mg once daily for one and two years recorded no drug-related serious adverse events and no clinically meaningful ECG or laboratory abnormalities; adverse events at ≥5% were dyspepsia, headache and back pain. NAION sits at HR 1.31 (0.88–1.94) — not significant, and 8.4 against 5.1 events per 100,000 person-years, so roughly three excess events per 100,000 person-years even taken as causal.

**Where it touches the rest of this document.** Tadalafil lowers systolic pressure by about 2.8 mmHg pooled, up to ~5/4 mmHg in men not already on an antihypertensive. Three things follow. Part of whatever real cardiovascular effect exists here is probably just that, and [the blood pressure row](#blood-pressure-aim-105115-systolic-drug-over-120) buys the same millimetres with far better evidence. The hypotension is additive with an FMD cycle or a poor-intake tirzepatide week, which makes it a symptom-watch during fasting rather than the formal hold rule an ARB needs. And a 17.5 h half-life makes it a state rather than a transition — but it carries no mTOR or IGF-1 valence at all, so it is genuinely phase-neutral and can run continuously through both blocks, which is more than metformin or rapamycin manage.

**Nothing to gain on the training rows at 38.** Tadalafil does not change VO₂max or aerobic and anaerobic thresholds in trained athletes in normoxia, and PDE5 inhibition raises blood flow to contracting skeletal muscle in older but not young subjects. The effect that might eventually matter is age-dependent, and it is decades away.

**The ITP is testing the other molecule.** 240 mice began sildenafil-laced chow in late 2024, 80 at each of the three sites against 160 controls, which puts a lifespan readout around 2030–2031. Nomination is a statement about priors rather than a result: [astaxanthin](#astaxanthin) and [Ca-AKG](#calcium-alpha-ketoglutarate-ca-akg) sit at 2 and 1 on this list precisely because the ITP tested them. Sildenafil in chow is at least the right *exposure model* for a daily protocol, but half-life, brain penetration and selectivity differ enough that a hit would not transfer to tadalafil cleanly.

**What would change the score.** A male-specific ITP hit takes it to E3 immediately. Any randomised trial with a hard cardiovascular endpoint settles it in either direction. And if erectile dysfunction, LUTS/BPH or pulmonary arterial hypertension ever appears, this becomes first-line and genuinely good — the same move [SGLT2 inhibitors](#dapagliflozin--empagliflozin--canagliflozin) get, where the trigger changes from aging to diagnosis.
...
**Why:** E2 — a specific, well-characterised mechanism plus consistent cohorts, and RCTs that reliably move the biomarker without moving the endpoint. B1 — the endpoint it aims at is arterial calcification, and at 38 with ApoB handled there is nothing there yet to act on.
...
**3 × 4 = 12** · OK · low · Rx, age 50+ · **CALENDAR ITEM — not eligible at 38**
...
**Why:** E4 — efficacy RCTs are solid, and post-MI trials (IAMI) showed reduced all-cause death, hinting at cardiovascular benefit beyond infection itself. B2 — modest for a healthy 38-year-old. Belongs on the list mainly because the vaccine-and-dementia literature that makes Shingrix interesting is not obviously zoster-specific.

#### HPV vaccination
**4 × 2 = 8** · OK · low · Rx, approved through 45
**Target:** oncogenic HPV · **Cautions:** none
**Why:** E4 — RCTs on cancer-precursor endpoints plus registry data on cancer incidence. B2 — at 38 you have likely already been exposed, so residual benefit is modest. Scores low, but the window shuts at 45 and rising male oropharyngeal cancer makes it a reasonable cheap bet.
...
**Why:** E3 — strong and consistent observational plus Mendelian randomisation, but sleep-extension RCTs with hard outcomes do not exist. B5 on the second clause of the [benefit scale](anti-aging-inventory.md#the-scoring-model) — nothing else on this list works properly if this is broken, and a bad week of it degrades training output, appetite regulation, mood and blood pressure simultaneously. Reallocated time, not extra time.
...
**Why:** E1 — one unreplicated mouse study and one single-arm human series, against two ITP nulls. B1 — no route to a measurable outcome at 38.
