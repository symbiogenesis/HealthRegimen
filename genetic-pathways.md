...
- **As a therapeutic proposal,** it is nearly worthless on its own. It says nothing about whether editing *PCSK9* in a 38-year-old's hepatocytes is safe, how much of the 88% survives the timing change, or what the edit does over forty years that a stopped drug would not.
...
### The attenuation ladder — what a germline effect size is worth at 38

A Mendelian randomization estimate describes a variant that has been running since conception in every cell. Five discounts separate that number from what a somatic edit buys a 38-year-old, and they compound.

**1. Exposure time.** This is the largest, and the repo already has the arithmetic. Ference puts 40 years of 1 mmol/L lower LDL-C at 54.5% lower CHD risk, against roughly 24% for a statin started mid-life and run five years. Same molecule, same target, ~3× the effect from starting earlier. **You cannot buy back the first 38 years with an edit.** What an edit at 38 delivers is the *start-now* number — which is the same number a $5 statin started now delivers. The exposure-time leverage that makes the genetics so persuasive is the one component that is not for sale.
...
`E` = evidence (1–5) for **the edit, in you, now** · `B` = benefit at 38 · safety is a veto flag · effort sits outside the score.
...
**The drug:** [rosuvastatin + ezetimibe](therapeutics-by-mechanism.md#2b-hmg-coa-reductase) (25 and 16) reach most of this for $12–15/month; [evolocumab](therapeutics-by-mechanism.md#evolocumab--alirocumab) (25), [inclisiran](therapeutics-by-mechanism.md#inclisiran) (20) and [enlicitide](therapeutics-by-mechanism.md#enlicitide-decanoate-lipfendra-oral) (20) hit PCSK9 itself.
...
**The drug:** [dapagliflozin](therapeutics-by-mechanism.md#3b-sglt2-inhibition) (6, MONITOR, optional). Canagliflozin gave +14% median lifespan in male ITP mice, the largest male effect in ITP history; dapagliflozin is the agent the drug row picks, on the non-diabetic human trials and on being the only one of the three with a generic.
...
**Why:** E1 · B1 · AVOID — you cannot install this at 38, and the cohort that has it lifelong shows what it buys and what it does not. **Its real use in this repo is as a check on Part 4:** it is the strongest human evidence that suppressing growth signalling protects against cancer and diabetes, and simultaneously the strongest human evidence that doing so does not straightforwardly extend life. Both halves belong in the ledger, and the second half is the one that keeps the [rhGH row](therapeutics-by-mechanism.md#rhgh-containing-pro-growth-block-triim--triim-x-style) at MONITOR rather than AVOID.
...
**The natural experiment — weak, and the honest thing to say is that this pathway is not really a genetics story.** There is no protective human *PDE5A* knockout cohort. What exists is drug-target MR on cis-eQTLs for *PDE5A*, where higher enzyme expression carries CHD OR 1.22 (1.06–1.40) and MI OR 1.27 (1.09–1.48) with replication in aortic tissue — the right direction for inhibition, on a single-gene instrument with acknowledged pleiotropy. Around it sits a large and messy observational literature: lower mortality after myocardial infarction in registry data, and a much-publicized claims-based association with reduced Alzheimer's incidence that **did not survive better-controlled replication**.
**Why:** E1 for the edit and E2 at best for the pathway. B1 for the edit — **tadalafil is a generic pill costing a few dollars a month, and any test of this hypothesis runs through it**, so the drug takes essentially all of the achievable benefit and leaves the edit nothing to add. That drug carries [its own inventory row at 4](therapeutics-by-mechanism.md#tadalafil-low-dose-daily), which is where the cohort data, the null randomised record and the ITP sildenafil readout are worked through; the pathway is cheap enough to act on and not strong enough to change a decision. One live interaction to carry regardless: berberine raises tadalafil exposure through CYP3A4 and P-gp, which is part of why berberine is an [AVOID](therapeutics-by-mechanism.md#4-ampk-and-energy-sensing).
...
**Why:** E1 — the first real human test of the intended direction was negative. B1 — no route to benefit at 38 from a pathway whose agonism has not been shown to do anything.
...
**The practical position:** nothing to do at 38. If ancestry makes V122I plausible, that genotype is worth knowing eventually — after the insurance is in place — and the trigger is unexplained left ventricular hypertrophy, heart failure with preserved ejection fraction, bilateral carpal tunnel syndrome or lumbar spinal stenosis, not a birthday. **One durable caution for the whole class:** TTR carries thyroxine and retinol-binding protein, so knockdown requires vitamin A monitoring.

### 2.8 Cancer predisposition — a different kind of row entirely

*BRCA1*, *BRCA2*, *PTEN* and *TP53* are not "install a beneficial variant." They are "remove a deleterious one you may not have," and three things follow that do not apply anywhere else on this page. The separate proposal — *adding* tumor suppressor capacity to someone who carries no mutation at all — is a different argument with a different failure mode, and it is in [§2.11](#211-petos-paradox--the-cross-species-transplants).
...
**Second, tumor suppressors are the all-or-nothing case.** [Part 1's coverage discount](#the-attenuation-ladder--what-a-germline-effect-size-is-worth-at-38) is fatal here. Editing 60% of hepatocytes gives you 60% of a PCSK9 effect, because circulating protein is a dose. Correcting 60% of cells in a *BRCA1* carrier gives you approximately none of the benefit, because a tumor arises from **one** uncorrected cell that loses its remaining allele. The target is every cell in every at-risk tissue, and it is a *correction*, not a knockout — the hardest tier of the hardest problem.

**Third, the effective intervention already exists and is not genetic.** For a male *BRCA2* carrier the actionable consequences are earlier and more intensive prostate surveillance, awareness of male breast and pancreatic risk, PARP-inhibitor sensitivity if cancer ever arises, and — the part that usually matters most — implications for children and siblings. For *PTEN*, Cowden syndrome carries a distinctive phenotype (macrocephaly, mucocutaneous lesions, thyroid and renal findings) that is typically recognizable well before 38, and management is a surveillance schedule. For *TP53* the non-genetic intervention is the strongest on this page by some distance, and it is described in the row below.
...
#### TP53 correction (Li-Fraumeni)
**1 × 1 = 1** · MONITOR · high · no program · **the surveillance protocol is the whole row**
**The natural experiment — the most penetrant cancer predisposition in medicine.** Germline heterozygous *TP53* loss of function causes Li-Fraumeni syndrome: sarcoma, premenopausal breast cancer, brain tumors, adrenocortical carcinoma and leukemia, with lifetime cancer risk in the 70–90% range and a substantial fraction of it arriving before 45. Prevalence is on the order of 1 in 3,000–5,000, higher than intuition suggests, and population sequencing keeps finding lower-penetrance carriers who never met classic family-history criteria. *TP53* is also the most frequently mutated gene in sporadic human cancer, which is a separate fact about somatic biology and not evidence about the germline edit.
**Why the correction is not the answer:** E1 · B1, on the [§2.8 reasoning](#28-cancer-predisposition--a-different-kind-of-row-entirely) and harder. Li-Fraumeni's at-risk tissue list is essentially the whole body, so "every cell in every at-risk tissue" is literal here, and it is a precise correction rather than a knockout.
**What actually works, and it is unusual for this document to be able to say so:** the Toronto protocol — annual whole-body MRI, brain MRI, breast MRI, plus interval biochemistry and abdominal ultrasound — produced a large survival separation in prospective follow-up of *TP53* carriers, on the order of 5-year overall survival near 90% under surveillance against roughly 60% without. That is a bigger absolute effect than anything a drug on this page delivers, it requires no editing, and it is the same [early detection](therapeutics-by-mechanism.md#16-early-detection) row (20) that already sits near the top of the inventory, run at higher intensity.
**Two consequences that bind even before you know your status.** First, carriers are radiation-sensitive, so the protocol is deliberately MRI-based and diagnostic CT is minimized — worth holding alongside [coronary artery calcium](therapeutics-by-mechanism.md#coronary-artery-calcium), which is a radiation-delivering scan whose score would change for a carrier. Second, insure before testing: **GINA does not cover life, disability or long-term-care insurance**, and *TP53* is the genotype most likely to be underwritten against. Testing is triggered by family history meeting Chompret-style criteria — early sarcoma, premenopausal breast cancer, adrenocortical carcinoma, childhood brain tumor — not by curiosity.
...
### 2.11 Peto's paradox — the cross-species transplants

Every other row on this page proposes installing a variant that some humans already carry. This family proposes installing something **no human has ever had**, taken from a species that solved a problem we did not have to solve.

The problem is Peto's paradox: cancer risk should scale with cell count and lifespan, and across species it does not. A 4-tonne elephant living seventy years should be riddled with tumors and instead dies of cancer at roughly a third to a quarter the human rate. Large, long-lived lineages each evolved an answer, and the answers are **different in every lineage** — elephants amplified *TP53*, naked mole rats secrete high-molecular-mass hyaluronan and contact-inhibit early, blind mole rats trigger concerted interferon-β cell death, bowhead whales duplicated DNA-repair genes including *ERCC1* and *PCNA*. Convergence on the outcome with no convergence on the mechanism is the signature of solutions that are **local to a genome**, not portable parts.

That is the framing objection to this whole family, and it is not the same objection as the rest of the page. Elsewhere the problem is delivery or exposure time. Here the problem is that the evidence is entirely comparative — the variant has never run in a human, so there is no human natural experiment at all, and Part 1's [compensation discount](#the-attenuation-ladder--what-a-germline-effect-size-is-worth-at-38) applies at full strength with nothing on the other side of it.

#### TP53 copy amplification
**1 × 1 = 1** · **AVOID** · high · no program
**The comparative evidence — striking, and it is comparative.** Elephants carry roughly 20 copies of *TP53* (one ancestral gene plus ~19 retrogene copies), against one in humans, and their lymphocytes respond to DNA damage by **dying readily rather than attempting repair** — a lowered apoptotic threshold, not better repair fidelity. Cancer mortality in elephants runs around 5% against 11–25% in humans. There is no human cohort with extra *TP53*; the closest human genetics offers is the codon 72 Pro/Arg polymorphism, where the more apoptosis-prone Arg allele and the more arrest-prone Pro allele have been chased through longevity cohorts for two decades with contradictory results — one 85-plus cohort found Pro/Pro homozygotes surviving longer *despite* higher cancer mortality. That inconsistency is the finding: humans have no clean "more p53" natural experiment.
**The mouse work has already run the experiment, and it found the trade-off.** Transgenic mice with extra *TP53* copies under normal regulation are tumor-resistant and age normally — but they do not live longer. Mice with a **hyperactive** p53 allele are also tumor-resistant and age *prematurely*, with roughly 20% shorter lifespan, stem cell depletion, osteoporosis and reduced tissue regeneration. Lifespan extension appears only when extra p53 is combined with extra *Arf* and delivered under intact endogenous control, and then it is modest. The dose window is narrow, the failure mode on the high side is accelerated aging, and **regulation matters more than copy number.**
**The drug says the same thing in humans.** MDM2 inhibitors — idasanutlin, navtemadlin, milademetan, brigimadlin — are the pharmacological version of "more p53 in a *TP53*-wild-type cell," and their dose-limiting toxicity is thrombocytopenia and neutropenia, because hematopoietic stem cells are the most p53-sensitive compartment in the body. That is the mouse premature-aging phenotype appearing in humans within weeks. Phase 3 results in the class have been poor, and there is a standing concern that pushing p53 pressure selects for *TP53*-mutant clones — the same clonal-hematopoiesis biology that makes *TP53* CHIP one of the highest-risk clones a person can carry.
**Why:** E1 — no human evidence of any kind for the intervention, and the closest pharmacological proxy has a marrow ceiling. B1 — cancer is a real target, but this is a whole-body overexpression job in a tumor suppressor, which stacks Part 1's [coverage](#the-attenuation-ladder--what-a-germline-effect-size-is-worth-at-38) and delivery discounts on top of each other: partial coverage buys approximately nothing, and AAV is episomal so it dilutes out of exactly the dividing compartments you would be trying to protect. **AVOID is about the [control problem](#the-control-problem--an-edit-deletes-the-levers-this-regimen-runs-on), and it is the sharpest instance of it on the page.** The intervention's benefit and its harm are the same knob at different settings, the harm accrues silently over decades as stem cell exhaustion, and an edit has no dose to cut and no washout. Everything the regimen's [growth cycling](anti-aging-inventory.md#part-4--growth-cycling) does by scheduling — press anti-growth, then release it — this makes permanent in one direction.

#### LIF6 (the elephant apoptosis effector)
**1 × 1 = 1** · MONITOR · high · no program · **the name is the trap**
**What it actually is.** Elephants carry 7–11 *LIF* pseudogene copies. One of them, *LIF6*, acquired a **p53 response element** and became transcribable again — a "zombie gene." Its product is not a secreted cytokine: it translocates to mitochondria, permeabilizes them and induces apoptosis after DNA damage. It is the effector arm of the amplified *TP53* above, which is why the two are one story and not two.
**Why it is graded separately from the *TP53* row, and slightly better.** *LIF6* is the only construct on this page with **regulation built into the construct** — it fires downstream of p53 rather than constitutively. That is a genuinely interesting design, and it is the shape a good edit would have: the duty cycle is in the biology instead of in a dosing schedule. It is also the only thing it has going for it.
**The trap, and it is why this question gets asked.** Human *LIF* is not *LIF6*. Canonical LIF is a pleiotropic IL-6-family cytokine signalling through LIFR/gp130 to STAT3 — the same gp130 as the [IL6R row](#210-inflammation--il6r-loss-of-function) — and it is required for blastocyst implantation, maintains stem cell pluripotency, and in human tumor biology is **pro-tumorigenic**: overexpressed in pancreatic, colorectal and breast cancer, driving immunosuppressive macrophage polarization and chemoresistance. The therapeutic direction humans have actually taken it is **blockade** — an anti-LIF antibody ran in early pancreatic cancer trials, and nothing in the class has read out positively. LIF is also a mediator of cancer cachexia through gp130 in muscle and adipose, which is the wrong direction entirely for a regimen tracking ALMI. On the loss side, biallelic *LIFR* loss of function causes Stüve-Wiedemann syndrome, frequently fatal in infancy. **"Overexpress LIF" and "install LIF6" are opposite interventions with the same three letters**, and the pathway is the cleanest example on this page of Part 1's [fifth discount](#the-attenuation-ladder--what-a-germline-effect-size-is-worth-at-38) — direction is not symmetric, and here it is not even shared between a gene and its own retrocopy.
**Why:** E1 — mouse and cell work in a non-human gene, with zero human data. B1 — and the residual worry is that a p53-driven mitochondrial permeabilizer, installed in an adult whose p53 already fires continuously against ordinary oxidative and replicative damage, is a stem cell depletion machine wearing the same trade-off as the *TP53* row. MONITOR rather than AVOID only because nothing exists to take and nobody is selling it; if a construct ever appeared, the burden is a demonstration that the p53 response element is tight enough to stay quiet at baseline.

**What this family is for.** It is the answer to "why not just take the parts that work in the animals that beat cancer." The parts are not modular, the human ortholog frequently runs the other way, and in the one case where the mechanism has been reproduced in a mammal — extra p53 in mice — the result was cancer resistance with **no lifespan gain**, or lifespan loss when the dose was pushed. For a 38-year-old the entire realizable version of this is [cancer screening on schedule](therapeutics-by-mechanism.md#16-early-detection) (20), which detects the tumors these genes would have suppressed, and is one of the highest-scoring rows in the repository.
...
| **1** | 1×1 | *PDE5* | loss of function | ok | the drug is generic and carries the row (4); the AD signal did not replicate |
| **1** | 1×1 | *APOC3* · *NPC1L1* | loss of function | ok | covered by diet and by ezetimibe (16) |
| **1** | 1×1 | *BRCA1* · *BRCA2* · *PTEN* · *TP53* | correction | monitor | not an existing capability; test, insure first, then screen — *TP53* surveillance has the largest survival effect on the page |
| **1** | 1×1 | *LIF6* | elephant retrogene transgene | monitor | no human data; human *LIF* is a blockade target, not an installation target |
...
| **1** | 1×1 | *TP53* extra copies | amplification | **AVOID** | hyperactive p53 shortens mouse lifespan; MDM2 inhibitors hit a marrow ceiling |
...
| **A first-degree relative has early sarcoma, premenopausal breast cancer, adrenocortical carcinoma or a childhood brain tumor** | *TP53* testing under Chompret-style criteria, insured first — this is the genotype most likely to be underwritten against. A positive result installs the Toronto protocol (annual whole-body and brain MRI) and makes diagnostic radiation something to spend deliberately, which changes the [coronary artery calcium](therapeutics-by-mechanism.md#coronary-artery-calcium) answer. |
